...
namespace tiramisu::auto_scheduler
{
    ...
    void beam_search::search_save(syntax_tree& ast, std::vector<std::string> *schedules_annotations, candidate_trace *parent_trace, float schedule_timeout)
    {
        ...
        std::chrono::steady_clock::time_point gen_end = std::chrono::steady_clock::now();

        std::cout << "#sched_gen time : " << std::chrono::duration_cast<std::chrono::milliseconds>(gen_start - gen_end).count() << " ms" << std::endl;
        ...
        while (iterator != children.end())
        {
            ...
            std::chrono::steady_clock::time_point transform_start = std::chrono::steady_clock::now();
            child->transform_ast();

            std::chrono::steady_clock::time_point transform_end = std::chrono::steady_clock::now();

            std::cout << "\n#transform ast time : " << std::chrono::duration_cast<std::chrono::milliseconds>(transform_start - transform_end).count() << " ms" << std::endl;

            std::chrono::steady_clock::time_point legal_start = std::chrono::steady_clock::now();

            bool is_legal = (*iterator)->ast_is_legal();

            std::chrono::steady_clock::time_point legal_end = std::chrono::steady_clock::now();

            std::cout << "#ast_is_legal time : " << std::chrono::duration_cast<std::chrono::milliseconds>(legal_start - legal_end).count() << " ms" << std::endl;
