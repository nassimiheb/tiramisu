...
std::vector<syntax_tree *> ml_model_schedules_generator::generate_matrices(syntax_tree &ast)
{
    ...
    if(explre_interchange){
        ...
        for (int i = 0; i < shared_nodes.size(); ++i)
        {
            for (int j = i + 1; j < shared_nodes.size(); ++j)
            {
                ...
                std::vector <  std::vector<int> >  matrix(2*depth+2);
                for(int l = 0; l<matrix.size(); l++){
                    matrix.at(l)= std::vector<int>(2*depth+2);
    ...
    //     std::cout<<"disable fusion this time"<<std::endl;
            ...
            { // similar itr domains
                ...
                if (shifting_res.size() > 0)
                {
                    ...
                    if(new_ast->ast_is_legal()){
                        ...
                        for (tiramisu::computation* current_comp : optim_info.comps) // iterate over the ordered computations list
                        {
