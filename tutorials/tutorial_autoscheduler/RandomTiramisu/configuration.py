...
#     else:
#         nb_child_loops = round(abs(local_random.gauss(mu=0, sigma=2.5)))
#     return nb_child_loops  # temporarily, to be changed later

def choose_nb_child_loops(loop):  # A temporarily implementation for generating single computation programs, the real implementation is commented above
    ...
    if loop.depth == 0:  # since currently we can't schedule programs with less than 2 shared loops, we need to force the generation of 2 shared loops
        ...
        nb_child_loops = min(round(abs(local_random.gauss(mu=0, sigma=1/2.5))), 1)  # either 0 or 1
    else:
        nb_child_loops = min(round(abs(local_random.gauss(mu=0, sigma=2.5))), 1)  # either 0 or 1
    return nb_child_loops  # temporarily, to be changed later
...
def choose_nb_child_comps(parent_loop):  # A temporarily implementation for generating single computation programs, the real implementation is commented above

    if parent_loop.nb_child_loops == 0:  # generate 1 comp at leaf loops
        return 1
    else:
        return 0
...
def choose_read_accesses(computation, program):  # chooses buffers to read and access pattern for each (list of ReadAccess objects)
    ...
    # if computation.absolute_order < 3:  # giving a bit more chance for the 3 first computations to be an initialization # for one comp programs, we don't want to generate a lot of init programs
    ...
    for buffer in candidate_buffers:
        if buffer is computation.write_access.buffer:
            ...
            buffer_selection_weights.append(len(buffer.defining_iterators)/len(computation.parent_iterators_list))  # give more proba to buffers with more dims

    selected_buffers = local_random.choices(candidate_buffers, weights=buffer_selection_weights, k=nb_buffers_to_read)

    # especially for 1 comp programs, if not all parent loops are dims in the write buffer, we must have a reduction access to avoid calculation overwriting
    if len(computation.parent_iterators_list) != len(computation.write_access.buffer.defining_iterators):
        if not (computation.write_access.buffer in selected_buffers):
            # should I randomly drop a buffer for the selected list to keep the same number ?, shouldn't  since it has a very litlle influence no the stats
            selected_buffers.append(computation.write_access.buffer)
    ...
    for _ in range(selected_buffers.count('new buffer')):
        ...
        while buffer_size > MAX_BUFFER_SIZE:  # re-pick the defining iterators until the buffer size is bellow the limit
            ...
            selected_iterators = random_slice(computation.parent_iterators_list, buffer_dim)
        ...
        if local_random.random() < 0.15:  # probability of reversing iterators order for the new buffer
            selected_iterators = selected_iterators[::-1]
    ...
    for buffer in selected_buffers:  # generate ?random? access matrices for each buffer, the current version is not random, maybe randomness can be generated for stencils
        ...
        if local_random.random() < 1/(len(selected_buffers)**2*1.33):  # probability of making a stencil, gives higher chances when number of buffers is small, gives 0.75 for 1, 0.2 for 2
            ...
            stencil_type = local_random.choices(['star', 'square', 'uni_dim'], weights=[0.4, 0.4, 0.25], k=1)[0]

            if stencil_type == 'star':  # star stencil over a selected set of dimensions {3dims(hea3d) or 2dims (jacobi2d)}
                # access_matrices_list = [access_matrix.copy() for _ in range(len(buffer.defining_iterators)+1)]
                if len(buffer.defining_iterators) >= 3:  # if more than 2 dims, we select a subset of them, else we take them all
                    nb_selected_dims = local_random.choice(list(range(2, len(buffer.defining_iterators)+1)))  # randomly choose nb dims to stencil
                    selected_dims = local_random.choice([list(range(len(buffer.defining_iterators)))[:nb_selected_dims], list(range(len(buffer.defining_iterators)))[-nb_selected_dims:]])  # either take the n first dims or the n last dims
                else:
                    selected_dims = list(range(len(buffer.defining_iterators)))
                ...
                for i in selected_dims:
                    ...
                    access_matrix_copy[i, -1] = -1
                    access_matrices_list.append(access_matrix_copy)
                # print(len(access_matrices_list))
            ...
            elif stencil_type == 'square':  # square stencil over a selected set dimensions {2dims (seidel2d)}
                if len(buffer.defining_iterators) >= 3:  # if more than 2 dims, we select a subset of them, else we take them all
                    nb_selected_dims = local_random.choice([2, 3])  # randomly choose nb dims to stencil
                    selected_dims = local_random.choice([list(range(len(buffer.defining_iterators)))[:nb_selected_dims], list(range(len(buffer.defining_iterators)))[-nb_selected_dims:]]) # either take the n first dims or the n last dims
                else:
                    selected_dims = list(range(len(buffer.defining_iterators)))
                access_matrices_list = [access_matrix]
        ...
        elif local_random.random() < 0.05:  # probability of making a partial stencil, i.e. one +1 or +2 to one of the dimensions
            ...
            # print(program.name, 'partial stencil')

        else:  # No stencil
            read_accesses_list.append(ReadAccess(buffer=buffer, access_pattern=access_matrix, computation=computation))

        # TODO add a chance for duplicating some randomly chosen memory accesses
    # print(program.name, 'nb_acc', len(read_accesses_list))
...
def choose_expression(computation, read_access_list):  # TODO format of expression not yet defined, using string temporarily
    ...
    if nb_read_accesses > 1:
        ...
        operator = local_random.choices(operators_list, weights=[0.50, 0.10, 0.25, 0.05], k=1)[0]
    ...
    while expression_list.count('E') < nb_read_accesses:
        ...
        expression_list[i:i+1] = local_random.choices([['E', operator, 'E'], ['(', 'E', operator, 'E', ')'],
                                                       ['(', 'E', operator, 'V', ')']], weights=[0.70, 0.20, 0.10], k=1)[0]
...
def random_slice(lst, k):  # randomly selects a list of k successive elements from lst
    start = local_random.randint(0, len(lst)-k)
...
def has_skippable_loop(computation):

    # print(computation.expression.expression)
    if computation.write_access.buffer.defining_iterators == computation.parent_iterators_list:  # all iterators used in writing
        return False
    if computation.write_access.buffer.wrapping_input.name not in computation.expression.expression:  # computation is clearly overwritten
        return True

    # find the  simple reduction accesses
    found = False
    for read_acc in computation.expression.read_access_list:
        if read_acc.buffer == computation.write_access.buffer and not read_acc.access_pattern_is_stencil():
            found = True  # a simple recution access is found
            if read_acc.write().replace(' ', '') not in computation.expression.expression.replace(' ', ''):  # the simple reduction access is not present in simplified expression,
                return True
            else:
                break
    if not found:  # no simple reduction access is found, but we know that there is a reduction access in expression, so there is a skippable loop if the reduction is performed on last iterator, otherwise it's hardly skippable
        if computation.write_access.buffer.defining_iterators[-1] != computation.parent_iterators_list[-1]:  # reduction performed on the last iterator
            return True

    # find the non simple reduction accesses
    for read_acc in computation.expression.read_access_list:
        if read_acc.buffer == computation.write_access.buffer and read_acc.access_pattern_is_stencil():
            if read_acc.write().replace(' ', '') in computation.expression.expression.replace(' ', ''):  # a stencil access pattern is used and is present in simplified expression
                return False

    # checking if there is a free loop (not used in write nor in read)
    used_iterators = []
    for read_acc in computation.expression.read_access_list:
        used_iterators.extend(read_acc.buffer.defining_iterators)

    used_iterators.extend(computation.write_access.buffer.defining_iterators)
    used_iterators = set(used_iterators)

    if len(used_iterators) == len(computation.parent_iterators_list):  # all iterators are used
        return False

    if computation.parent_iterators_list[-1] in used_iterators:  # the last iterator is not the dropped one, so the dropped loop shouldn't be skippable (knowing that there is a reduction access)
        if len(re.findall(r'\w+\(', computation.expression.expression)) > 2:  # has to have more than 2 accesses to make sure the loop isn't skippable, adding this condition for stricness, Edit: this does count expr( as an access but it's okay
            return False
